...
class Plugin(indigo.PluginBase):
	...
	def setUserPin(self, pluginAction):
		...
		self.debugLog("userPin: %s" % userPin)
		...
		if (bytes[7] == "71") and (bytes[8] == "05") and (bytes[9] == "00"): #COMMAND_CLASS_NOTIFICATION (aka ALARM) v3 = Lock Status
			...
			if (bytes[13] == "06"): #Access Control
				if (bytes[14] == "01"):
					...
					self.debugLog(u"User number: %d" % (int(bytes[16],16)))
...
########################################
	...
	def getName(self,inNode,inUser):
		...
		if (uList1 <> ""):
			if (int(uList1) in indigo.devices):
				...
				if inUser <= 10:
					uIsActive = list1Dev.ownerProps.get('tActive' + str(inUser), False)
					if uIsActive:
						outUser = list1Dev.ownerProps.get('tName' + str(inUser), u'Unknown User')
						return outUser
	...
	def getPin(self,userNo,indigoDev):
		node = self.nodeFromZed[int(indigoDev)]
		indigo.server.log("Getting PIN for " + self.getName(node,userNo))
