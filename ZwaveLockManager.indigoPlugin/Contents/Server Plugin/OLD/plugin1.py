...
# TODO
#
# Validate if user IDs are in higher priority lists
# Implement deviceChanged() for name changes
#
###########
...
########################################
	def zwaveCommandReceived(self, cmd):
		...
		if (int(bytes[5],16)) not in self.lockIDs:
			...
			self.debugLog(u"Node ID %s found in lockIDs" % (int(bytes[5],16)))
		...
		if (bytes[7] == "71") and (bytes[8] == "05") and (bytes[9] != "00"): #COMMAND_CLASS_ALARM v1/v2 = Lock Status
			...
			if (bytes[9] == "70"):
				indigo.server.log(u"Status: User %s updated [Node: %s]" % (self.getName(int(bytes[5],16),int(bytes[10],16)), int(bytes[5],16)))
			elif (bytes[9] == "71"):
				indigo.server.log(u"Status: Updating user %s failed - PIN already exists [Node: %s]" % (self.getName(int(bytes[5],16),int(bytes[10],16)), int(bytes[5],16)))
			elif (bytes[9] == "09"):
				indigo.server.log(u"Status: Deadbolt jammed on door [Node: %s]" % (int(bytes[5],16)))
				self.triggerEvent("deadboltJammed",int(bytes[5],16),"")
			elif (bytes[9] == "12"):
				indigo.server.log(u"Status: User %s locked door [Node: %s]" % (self.getName(int(bytes[5],16),int(bytes[10],16)), int(bytes[5],16)))
				self.triggerEvent("lockedByCode",int(bytes[5],16),int(bytes[10],16))
			elif (bytes[9] == "13"):
				indigo.server.log(u"Status: User %s unlocked door [Node: %s]" % (self.getName(int(bytes[5],16),int(bytes[10],16)), int(bytes[5],16)))
				...
				indigo.server.log(u"Status: User %s removed from door [Node: %s]" % (self.getName(int(bytes[5],16),int(bytes[10],16)), int(bytes[5],16)))
			elif (bytes[9] == "A1"):
				if (bytes[10] == "01"):
		...
		if (bytes[7] == "71") and (bytes[8] == "05") and (bytes[9] == "00"): #COMMAND_CLASS_NOTIFICATION (aka ALARM) v3 = Lock Status
			...
			if (bytes[13] == "06"): #Access Control
				if (bytes[14] == "01"):
					...
					indigo.server.log(u"Status: Door locked by user %s [Node: %s]" % (self.getName(int(bytes[5],16),int(bytes[16],16)),int(bytes[5],16)))
					self.triggerEvent("lockedByCode",int(bytes[5],16),int(bytes[16],16))
				elif (bytes[14] == "06"):
					indigo.server.log(u"Status: Door unlocked by user %s [Node: %s]" % (self.getName(int(bytes[5],16),int(bytes[16],16)),int(bytes[5],16)))
		...
		if (bytes[7] == "63") and (bytes[8] == "03"): #COMMAND_CLASS_USER_CODE = User Code status
			...
			if (bytes[10] == "01"): #Code is set/slot is occupied
				...
				indigo.server.log(u"Status:  User code %s is %s [Node: %s]" % (self.getName(int(bytes[5],16),int(bytes[9],16)), retCode, int(bytes[5],16)))
			else:
				indigo.server.log(u"Status:  User code %s is empty [Node: %s]" % (self.getName(int(bytes[5],16),int(bytes[9],16)),int(bytes[5],16)))
			self.debugLog(u"-----")
	...
	def testHex(self):
		...
		#self.zwaveCommandReceived(cmd)
		cmd = {'bytes': [0x01,0x0A,0x00,0x04,0x00,0x2C,0x04,0x71,0x05,0x12,0x02,0xFF], 'nodeId': None, 'endpoint': None} #Locked user 2
		self.zwaveCommandReceived(cmd)
...
########################################
	...
	def triggerEvent(self,eventType,eventNode,userNo):
		...
		for trigger in self.events[eventType]:
			...
			dNodeID = self.nodeFromZed[int(dAddress)]
			if (userNo <> ""):
	...
	def getName(self,inNode,inUser):
		devID = self.devFromNode[int(inNode)]
		outUser = ""
		#self.debugLog(str(indigo.devices[int(devID)]))
		uList1 = indigo.devices[int(devID)].states['userList1']
		if (uList1 <> ""):
			if (int(uList1) in indigo.devices):
				list1Dev = indigo.devices[int(uList1)]
				if inUser <= 10:
					uIsActive = list1Dev.ownerProps['tActive' + str(inUser)]
					if uIsActive:
						outUser = list1Dev.ownerProps['tName' + str(inUser)]
						return outUser

		uList2 = indigo.devices[int(devID)].states['userList2']
		if (uList2 <> ""):
			if (int(uList2) in indigo.devices):
				list2Dev = indigo.devices[int(uList2)]
				if inUser <= 10:
					uIsActive = list2Dev.ownerProps['tActive' + str(inUser)]
					if uIsActive:
						outUser = list2Dev.ownerProps['tName' + str(inUser)]
						return outUser

		uList3 = indigo.devices[int(devID)].states['userList3']
		if (uList3 <> ""):
			if (int(uList3) in indigo.devices):
				list3Dev = indigo.devices[int(uList3)]
				if inUser <= 10:
					uIsActive = list3Dev.ownerProps['tActive' + str(inUser)]
					if uIsActive:
						outUser = list3Dev.ownerProps['tName' + str(inUser)]
						return outUser

		outUser = "User " + str(inUser)
		return outUser

	def setUserListOneDoor(self,pluginAction):
		self.debugLog("setUserListOneDoor action called")
		devID = pluginAction.deviceId
		self.debugLog("Indigo lock selected: " + indigo.devices[int(devID)].name)

		userList = pluginAction.props["userList"]
		listSlot = pluginAction.props["listSlot"]
		userListName = indigo.devices[int(userList)].name

		self.setUserListToDoorLock(userList,listSlot,devID)

	def setUserListByGroup(self,pluginAction):
		self.debugLog("setUserListByGroup action called")
		devID = pluginAction.deviceId
		self.debugLog("Indigo lock group selected: " + indigo.devices[int(devID)].name)

		userList = pluginAction.props["userList"]
		listSlot = pluginAction.props["listSlot"]

		for i in range(1,11): #1-10
			doorID = indigo.devices[int(devID)].ownerProps["door" + str(i)]
			if (doorID <> ""):
				self.debugLog("Door " + str(i) + ": " + str(doorID))
				self.setUserListToDoorLock(userList,listSlot,doorID)

	def setUserListToDoorLock(self,userList,listSlot,devID):
		#self.debugLog("Userlist: %s" % (userList))

		userListName = indigo.devices[int(userList)].name

		#Update lock with user list
		dev = indigo.devices[int(devID)]
		dev.updateStateOnServer('userList' + listSlot, value=userList)
		dev.updateStateOnServer('userList' + listSlot + 'Name', value=userListName)

		#Call setPIN for each user
		listDev = indigo.devices[int(userList)]
		zedID = self.zedFromDev[int(devID)]
		for i in range(1,11):	#1 to 10
			uIsActive = listDev.ownerProps['tActive' + str(i)]
			uIsEnabled = listDev.ownerProps['tEnabled' + str(i)]
			#self.debugLog(str(i) + " active: " + str(uIsActive) + str(type(uIsActive)))
			#self.debugLog(str(i) + " enabled: " + str(uIsEnabled) + str(type(uIsEnabled)))
			uPin = listDev.ownerProps['tPin' + str(i)]
			if (uIsActive):
				#self.debugLog(str(type(uIsActive)) + str(type(uIsEnabled)))
				if (uIsEnabled):
					self.setPin(i,uPin,zedID)
				else:
					self.clearPin(i,zedID)

	def clearUserListOneDoor(self,pluginAction):
		self.debugLog("clearUserListOneDoor action called")
		devID = pluginAction.deviceId
		self.debugLog("Indigo lock selected: " + indigo.devices[int(devID)].name)
		...
		self.clearUserListFromDoorLock(userList,devID)

	def clearUserListByGroup(self,pluginAction):
		self.debugLog("clearUserListByGroup action called")
		devID = pluginAction.deviceId
		self.debugLog("Indigo lock group selected: " + indigo.devices[int(devID)].name)
		...
		for i in range(1,11): #1-10
			...
			if (doorID <> ""):
				self.debugLog("Door " + str(i) + ": " + str(doorID))
				self.clearUserListFromDoorLock(userList,doorID)

	def clearUserListFromDoorLock(self,userList,devID):
		#self.debugLog("Userlist: %s" % (userList))
		...
		for i in range(1,4):
			if (dev.states["userList" + str(i)] == userList):
				dev.updateStateOnServer('userList' + str(i), value="")
				dev.updateStateOnServer('userList' + str(i) + 'Name', value="")
	...
	def setPin(self,userNo,userPin,indigoDev):
		node = self.nodeFromZed[int(indigoDev)]
		self.debugLog("Node: " + str(node))
		indigo.server.log("Setting PIN for " + self.getName(node,userNo) + " to: " + str(userPin))
	...
	def clearPin(self,userNo,indigoDev):
		node = self.nodeFromZed[int(indigoDev)]
		indigo.server.log("Clearing PIN for " + self.getName(node,userNo))
